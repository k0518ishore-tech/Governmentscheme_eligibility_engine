...
const API_BASE_URL = window.SCHEMEGUIDE_API_URL
...
// ── Eligibility Rule API ──────────────────────────────────────
const EligibilityRuleAPI = {
  getByScheme: (schemeId) => apiFetch(`/eligibility-rules/${schemeId}`),
  getById: (id) => apiFetch(`/eligibility-rules/rule/${id}`),
  create: (data) => apiFetch('/eligibility-rules', { method: 'POST', body: JSON.stringify(data) }),
  update: (id, data) => apiFetch(`/eligibility-rules/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  delete: (id) => apiFetch(`/eligibility-rules/${id}`, { method: 'DELETE' }),
};

// ── Bookmark API ──────────────────────────────────────────────
const BookmarkAPI = {
  create: (schemeId, data = {}) => apiFetch('/bookmarks', { method: 'POST', body: JSON.stringify({ schemeId, ...data }) }),
  getAll: () => apiFetch('/bookmarks'),
  getById: (id) => apiFetch(`/bookmarks/${id}`),
  delete: (id) => apiFetch(`/bookmarks/${id}`, { method: 'DELETE' }),
};

// ── Search History API ────────────────────────────────────────
const SearchHistoryAPI = {
  record: (data) => apiFetch('/search-history', { method: 'POST', body: JSON.stringify(data) }),
  getAll: () => apiFetch('/search-history'),
  getById: (id) => apiFetch(`/search-history/${id}`),
  delete: (id) => apiFetch(`/search-history/${id}`, { method: 'DELETE' }),
};

// ── Recommendation API ────────────────────────────────────────
const RecommendationAPI = {
  getAll: () => apiFetch('/recommendations'),
  generate: () => apiFetch('/recommendations/generate'),
  getById: (id) => apiFetch(`/recommendations/${id}`),
};

// ── Notification API ──────────────────────────────────────────
const NotificationAPI = {
  getAll: () => apiFetch('/notifications'),
  getById: (id) => apiFetch(`/notifications/${id}`),
