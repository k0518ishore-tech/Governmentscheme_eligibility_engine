...
async function renderDashboard(container) {
  ...
  const recommended = SCHEMES.filter(s => checkEligibility(s, user).eligible).slice(0, 3);
  const profileValues = user?.profile || {};
  const profileFields = ['age', 'gender', 'state', 'annualIncome', 'education', 'occupation'];
  const profileCompletion = Math.round(profileFields.filter(field => profileValues[field] !== undefined && profileValues[field] !== '').length / profileFields.length * 100);
  let applications = [];
  let notifications = [];
  try {
    ...
    applications = (result.data.applications || []).map(app => ({
      ...app,
      schemeName: app.schemeName || app.schemeId?.name || 'Government scheme',
      appliedDate: app.createdAt,
      status: app.status || 'Submitted',
    }));
  } catch (error) { console.warn('[SchemeGuide] Could not load applications for dashboard.'); }
  try {
    const result = await NotificationAPI.getAll();
    notifications = (result.data.notifications || []).map(item => ({
      ...item,
      read: item.readStatus,
      time: item.createdAt ? formatDate(item.createdAt) : '',
    }));
    AppState.notificationCount = notifications.filter(item => !item.read).length;
  } catch (error) { console.warn('[SchemeGuide] Could not load notifications for dashboard.'); }
  ...
  container.innerHTML = appLayout('dashboard', `
      ...
      <div class="flex justify-between items-center flex-wrap gap-4">
        ...
        <div>
          <div style="background:rgba(255,255,255,0.1);border-radius:var(--radius-lg);padding:var(--space-4) var(--space-5);border:1px solid rgba(255,255,255,0.15)">
            ...
            <div style="font-size:var(--fs-xl);font-weight:800;color:white;margin-bottom:var(--space-2)">${profileCompletion}%</div>
            <div class="progress-bar progress-thin"><div class="progress-fill" style="width:${profileCompletion}%"></div></div>
    ...
    <div class="grid grid-4" style="gap:var(--space-4);margin-bottom:var(--space-8)">
      ${[
        ...
        { label: 'Schemes applied to', val: new Set(applications.map(app => String(app.schemeId?._id || app.schemeId || '')).filter(Boolean)).size, color: 'var(--clr-primary)', icon: Icons.folder, page: 'applications' },
        { label: 'Recommended', val: recommended.length, color: '#8B5CF6', icon: Icons.spark, page: 'recommendations' },
      ].map(s => `
...
function getNotifColor(type) {
  const map = { scheme: 'var(--clr-primary)', new_scheme: 'var(--clr-primary)', application: 'var(--clr-green)', application_update: 'var(--clr-green)', recommendation: '#8B5CF6', update: 'var(--clr-amber-dark)', system: 'var(--clr-text-muted)' };
  return map[type] || 'var(--clr-text-muted)';
}

function getNotifIcon(type) {
  const map = { scheme: Icons.schemes, new_scheme: Icons.schemes, application: Icons.checkCircle, application_update: Icons.checkCircle, recommendation: Icons.spark, update: Icons.info, system: Icons.bell };
