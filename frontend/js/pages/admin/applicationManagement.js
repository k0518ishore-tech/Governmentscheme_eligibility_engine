...
async function renderApplicationManagement(container) {
  try {
    ...
    APPLICATIONS = (result.data.applications || []).map(app => ({
      ...
      user: app.userId,
      schemeId: app.schemeId?._id || app.schemeId,
      schemeName: app.schemeId?.name || 'Government scheme',
      category: app.schemeId?.category || '',
      appliedDate: app.createdAt,
      lastUpdated: app.updatedAt,
      status: app.status,
      applicantDetails: app.applicantDetails || {},
    }));
    USERS = APPLICATIONS.map(app => app.user).filter(Boolean);
  } catch (error) {
    APPLICATIONS = [];
    showToast('Could not load applications', error.message || 'Check the server connection.', 'error');
  }
...
function renderAdminAppsTable(apps) {
  return `
    <table>
      ...
      <tbody>
        ${apps.map(app => {
          const user = USERS.find(u => u._id === app.userId || u.id === app.userId) || { name: 'Unknown', email: '' };
          return `
            <tr>
...
function showAppAdminDetail(id) {
  ...
  const user = app ? USERS.find(u => String(u._id || u.id) === String(app.userId)) : null;
  if (!app) return;
  ...
  showModal(`
    ...
    <div class="modal-body">
      ...
      ${Object.keys(app.applicantDetails || {}).length ? `<h5 style="color:var(--clr-navy);margin:var(--space-5) 0 var(--space-3)">Submitted information</h5><div class="grid grid-2" style="gap:var(--space-3);margin-bottom:var(--space-5)">${Object.entries(app.applicantDetails).map(([key, value]) => `<div style="padding:var(--space-3);background:var(--clr-bg-alt);border-radius:var(--radius-md)"><div style="font-size:var(--fs-xs);color:var(--clr-text-muted)">${escapeAdminHTML(key.replace(/([A-Z])/g, ' $1').replace(/^./, c => c.toUpperCase()))}</div><div style="font-size:var(--fs-sm);font-weight:600;color:var(--clr-navy)">${escapeAdminHTML(String(value))}</div></div>`).join('')}</div>` : '<p class="form-hint">No additional application details were submitted.</p>'}
...
async function updateAppStatus(id, newStatus) {
  try {
    ...
    await renderApplicationManagement(document.getElementById('page-content'));
  } catch (error) {
    showToast('Could not update application', error.message || 'Please try again.', 'error');
  }
}
