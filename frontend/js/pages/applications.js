...
async function renderApplications(container) {
  ...
  const schemesAppliedCount = new Set(apps.map(app => String(app.schemeId || '')).filter(Boolean)).size;
  ...
  container.innerHTML = appLayout('applications', `
    ...
    <div class="grid grid-4" style="gap:var(--space-4);margin-bottom:var(--space-6)">
      ${[
        { label: 'Schemes applied', count: schemesAppliedCount, color: 'var(--clr-primary)', bg: 'var(--clr-primary-light)' },
    ...
    ${apps.length === 0 ? `
      <div class="empty-state">
        ...
        <p class="empty-state-msg">Find a scheme, review its official requirements, and send an application request to the SchemeGuide admin team.</p>
