...
function checkEligibility(scheme, userData) {
  ...
  // Age
  if (el.age && userData.age) {
    const met = userData.age >= el.age.min && userData.age <= el.age.max;
    results.push({
      criterion: 'Age',
      yours: `${userData.age} years`,
      required: `${el.age.min}–${el.age.max} years`,
      met,
    });
  ...
  // Gender
  if (el.gender && !el.gender.includes('All') && userData.gender) {
    const met = el.gender.includes(userData.gender);
    results.push({
      criterion: 'Gender',
      yours: userData.gender,
  ...
  // Income
  if (el.income && userData.income) {
    const met = userData.income <= el.income.max;
    results.push({
      criterion: 'Annual Income',
      yours: formatCurrency(userData.income),
  ...
  // Occupation
  if (el.occupation && !el.occupation.includes('All') && userData.occupation) {
    const met = el.occupation.includes(userData.occupation);
    results.push({
      criterion: 'Occupation',
      yours: userData.occupation,
  ...
  // Community
  if (el.community && !el.community.includes('All') && userData.community) {
    const met = el.community.includes(userData.community);
    results.push({
      criterion: 'Community',
      yours: userData.community,
  ...
  // State
  if (el.state && !el.state.includes('All States') && userData.state) {
    const met = el.state.includes(userData.state);
    results.push({
      criterion: 'State',
      yours: userData.state,
...
function getRecommendations(baseSchemeIds = []) {
  const viewed = baseSchemeIds.length ? baseSchemeIds : [1, 3, 7];
  return SCHEMES.filter(s => !viewed.includes(s.id))
    .map(s => ({
      ...s,
...
function getRecommendationReason(scheme) {
  const reasons = [
    `Similar to schemes you've recently explored.`,
    `Matches your profile and interests.`,
    `Popular in your state and age group.`,
    `You may benefit from this ${scheme.category} scheme.`,
    ...
    <footer class="pub-footer">
      <div class="container">
        <div class="disclaimer">
          <strong>Disclaimer:</strong> SchemeGuide is an independent information platform to help citizens discover government schemes.
        ...
        <div class="footer-top">
          ...
          <div>
            ...
            <div class="footer-links">
              <span class="footer-link">About SchemeGuide</span>
        ...
        <div class="footer-bottom">
          <span>© 2024 SchemeGuide. Information purpose only.</span>
